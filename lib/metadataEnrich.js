...
import { getCinemetaMetadata } from './cinemeta_api.js';
import { animeData } from './kitsu_api.js'

async function ImdbToKitsu(id, type) {
  const [imdbId, season, episode] = id.split(":")
  const kitsuMeta = await getCinemetaMetadata(imdbId, type)
    .then((metadata) => enrichImdbMetadata(metadata, animeData))
    .then((meta) => ({ meta: meta}))
  if (type == "movie") {
    const kitsuId =
...
export { enrichKitsuMetadata, enrichImdbMetadata, hasImdbMapping, ImdbToKitsu };